...
function displayBooks(books) {
  if (bookList) {
    bookList.innerHTML = "";
    books.slice(0, booksPerPage).forEach((book, index) => {
...
function createBookCard(book) {
  ...
  card.innerHTML = `
        <img src="${book.formats["image/jpeg"] || "preview.png"}" alt="${
    book.title
  }" loading="lazy">
        <div class="book-info">
...
async function displayWishlist() {
  ...
  wishlistBooksData.forEach((book, index) => {
    const bookCard = createBookCard(book);
    wishlistBooks.appendChild(bookCard);

    // Add a delay  animation
    setTimeout(() => {
      bookCard.classList.add("visible");
    }, index * 100);
  });
